...
# Parameter Umum
EDGE_THRESHOLD = 60         # Ambang batas deteksi tepi (0-255)

# Parameter Adaptive (DBSCAN & Clustering)
ADP_EPS = 6                 # Jarak maksimum antar sampel
...
class StegoAutomatedTester:
    ...
    def run_payload_stress_test(self):
        ...
        for pct in payload_percentages:
            ...
            print(f"   Progress: {pct}% Payload | Std PSNR: {p_s:.2f} | Adp PSNR: {p_a:.2f}")
    ...
    def run_robustness_test(self):
        ...
        for attack_name, attack_func in attacks:
            ...
            print(f"   Attack: {attack_name} | Std BER: {ber_std:.2f}% | Adp BER: {ber_adp:.2f}%")
    ...
    def visualize_results(self):
        ...
        plt.bar(x - width/2, df_rob['Std_BER'], width, label='Standard Edge', color='red', alpha=0.7)
        plt.bar(x + width/2, df_rob['Adp_BER'], width, label='Adaptive Edge', color='blue', alpha=0.7)
